	...
	implements Cloneable
{
	...
	private static final int port = 5523;
	private void fireUdpPackage(String action)
	{
		List<String> actions_list = Arrays.asList("up", "down");
		if (!actions_list.contains(action)) return;
		new Thread (() -> {
			try {
	...
	private Object physFromBuffer(char[] buffer)
	{
		...
		try
		{
			if (cnv != null && cnv[cnvSystem] != null)
			{
				...
				if (pid == 0x42 && ((float) result) > 35.0f) {
					fireUdpPackage("down");
				} else if (pid == 0x49 && ((float) result) > 35.0f) {
					fireUdpPackage("up");
				}
			}
			else
		...
		{
			result = "n/a";
			log.warning(String.format("[NDA]%s: %s - [%s]", toString(), ex.getMessage(), ProtUtils.hexDumpBuffer(buffer)));
...
private StackTraceElement stElm;
	/**
	...
	public long updatePvFomBuffer(char[] buffer)
	{
		try
		{
			...
			if(currErrorCount < MAX_ERROR_COUNT)
			{
				...
				log.warning(String.format("[NDA]%02X %-30s %16s %s",
										pid,
										label,
			...
			{
				log.warning(String.format("[NDA]Item disabled: %s (%d/%d)",
										  toString(),
										  currErrorCount,
										  MAX_ERROR_COUNT));
			}
