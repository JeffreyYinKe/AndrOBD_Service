	...
	implements TelegramListener, TelegramWriter, Runnable
{
	...
	public synchronized int handleTelegram(char[] buffer)
	{
		...
		log.warning("[NDA]ELM rx:'" + bufferStr + "' (" + lastTxMsg + ")");
	...
	{
		if (pid_list == null || pid_list.isEmpty()) {
			return super.getNextSupportedPid();
		}
		return pid_list.get(pid_idx++%pid_list.size());
	}

	private void setObdCommandFilter()
	{
		pid_list = new ArrayList<>();
		try {
			URL resourceUrl = getClass().getResource(command_filter_file);
			if (resourceUrl == null) {
				throw new IOException("Resource not found: " + command_filter_file);
			}
			...
			while ((line = br.readLine()) != null) {
				line = line.trim();
				if (!line.isEmpty()) {
					pid_list.add(Integer.parseInt(line.replace("0x", ""), 16)); // Convert hex to int
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
		log.fine("[NDA]pid_list :" + pid_list.size());
	}
	private int pid_idx = 0;
	private List<Integer> pid_list;
	private static final String command_filter_file = "res/canbus_commands_filter.txt";
}
